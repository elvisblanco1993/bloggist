<div>
    <div class="py-24 px-10 bg-black text-white text-3xl text-center font-bold uppercase">
        {{ __("Articles by") . ' ' . $author->name }}
    </div>

    <div class="my-12 max-w-5xl mx-auto px-10">
        <section class="my-8 grid grid-cols-6 gap-8">
            @forelse ($author->articles as $article)
                <article @class([
                    ...
                    <a href="{{ route('author.show', ['author' => $author->slug]) }}" class="mt-2 text-sm font-medium text-gray-800">{{ __("By") . ' ' . $author->name }}</a>
